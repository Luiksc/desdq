...
@onready var piel_karau_asustado = $karau_asustado
@onready var anima_karau_asustado= $karau_asustado/AnimationPlayer
...
func moviminto(delta: float) -> void:
	...
	if input_direccion.length() > 0.1:
		...
		piel_karau_asustado.rotation.y= lerp_angle(piel_karau_asustado.rotation.y, (atan2(direccion.x, direccion.z))+ 3*PI/2, LERP_VAL)
